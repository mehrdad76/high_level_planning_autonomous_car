...
def writeEndJump(stream):
    ...
    stream.write('\t\t_cont_m2 ->  m_int_pl\n')
    ...
    stream.write('\t\t_cont_m2 ->  m_int_pr\n')
    ...
    stream.write('\t\t_cont_m2 ->  m_int_hl\n')
    ...
    stream.write('\t\t_cont_m2 ->  m_progress\n')
    stream.write('\t\tguard { clock = ' + str(TIME_STEP) + ' k = ' +
                 str(NUM_STEPS-1) + ' y2 >= ' + str(18-0.2) + '}\n')
    stream.write('\t\treset { ')
    stream.write('clock\' := 0')
    stream.write('}\n')
    stream.write('\t\tinterval aggregation\n')
...
def writeComposedSystem(filename, initProps, dnn,
    ...
    with open(filename, 'w') as stream:
        ...
        writeEndMode(stream, plant_states, 'm_int_pr')
